...
def run():
    ...
    for folder in os.listdir('./../GTV-Database-UPC/'): #loop por todas las carpetas de imágenes
        ...
        for frame in imgs:
            ...
            faces = face_cascade.detectMultiScale(gray_image, 1.0485258, 5,  minSize=(70, 70)) #1.04 es el scaling de la imagen con respecto a las imagenes con las que se entrenó el modelo, 12 es el minNeighbours (afecta a la calidad y al numero de detecciones)
            ...
            if faces is ():
                ...
                profile_faces = profile_face_cascade.detectMultiScale(gray_profile_image, 1.0485258, 3, minSize=(70, 70)) #1.04 es el scaling de la imagen con respecto a las imagenes con las que se entrenó el modelo, 12 es el minNeighbours (afecta a la calidad y al numero de detecciones)
                ...
                if profile_faces is (): #No ha detectado un perfil izquierdo, así que la imagen podría estar del perfil derecho (Haar Cascade no reconoce perfiles derechos)
                    ...
                    profile_faces = profile_face_cascade.detectMultiScale(gray_flipped_image, 1.0485258, 3,  minSize=(70, 70)) #1.04 es el scaling de la imagen con respecto a las imagenes con las que se entrenó el modelo, 12 es el minNeighbours (afecta a la calidad y al numero de detecciones)
                ...
                #cv2.rectangle(frame,(x,y),(x+w,y+h), (255,102,34), 4) # el parámetro 4 representa el grosor de la línea del rectángulo
                    if flip_flag:
                        ...
                        detected_profile_face = frame[y:y+h, x:x+w] #hace el crop de la cara detectada (el interior del rectángulo)
                profile = True
