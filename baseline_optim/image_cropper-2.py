...
def detect_face(frame, net):
    ...
    try:
        ...
        frame = cv2.resize(frame, (240,240), cv2.INTER_LINEAR)
...
cropped_folder_path = './Cropped-IMGS-2/'
...
def run():
    ...
    for folder in os.listdir('./../GTV-Database-UPC-2/'): #loop por todas las carpetas de imágenes
        ...
        for frame in imgs:
            ...
            cnt += 1
            #detected_face = cv2.resize(detected_face, (240,240), interpolation=cv2.INTER_LINEAR)
